...
function gameStart() {
  ...
  catContent += "</ul>";
  categoriesDiv.innerHTML = `<p>Pick a category :</p> ${catContent}`;
}
...
function displayPicked(guess, chosenCat) {
  ...
  livesDiv.innerText = ` You have ${lives} lives left`;
...
function check(letter, wordToGuess) {
  ...
  if (wordToGuess.indexOf(letter) < 0) {
    ...
    livesDiv.innerText = ` You have ${lives} lives left`;
    drawHangMan(lives);
    if (lives === 0) endGame("loser");
  }

  if (correctAnswers === totalToWin) endGame("winner");
}
...
function endGame(txt) {
  txt === "winner"
    ? (livesDiv.innerText = `Congratulations! You won.`)
    : (livesDiv.innerText = `You lose!`);
